...
def outreg(regs, var_names, var_labels, digits=4, stars=True, se="(",
    ...
    Args
    ----
    `regs`, `Results` or iterable of `Results`: Regressions to
        output to table.
    `var_names`, str or iterable of str: Variable names to pull from `regs`.
    `var_labels`, str or iterable of str: Names for variables in table (i.e.,
        pretty print).

    Kwargs
    ------
    `digits`, int, 4: How many digits to include past decimal.
    `stars`, bool, True: If True, adds stars to mark statistical significance.
    `se`, str, "(": Marker for standard errors. Default is parens, may also
        choose brackets with `se="["`.
    `options`, bool, False: If True, return a `dict` with formatting options
        that were generated by `outreg`: `name_just`, `stat_just`, etc., for
        additional calls to `table_mainrow` and `table_statrow`.

    Return
    ------
    `table_str`, str: Latex fragment mean to be wrapped in a tabular
        environment.
    """
